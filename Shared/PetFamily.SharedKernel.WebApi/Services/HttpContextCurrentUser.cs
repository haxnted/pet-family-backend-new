...
internal sealed class HttpContextCurrentUser : ICurrentUser
{
	...
	/// <inheritdoc/>
	public bool HasRole(string role) => _contextAccessor.HttpContext?.User.HasRole(role) == true;

	/// <inheritdoc/>
	public bool IsAdmin => _contextAccessor.HttpContext?.User.IsAdmin() == true;

	/// <inheritdoc/>
	public IEnumerable<string> Roles => _contextAccessor.HttpContext?.User.GetRoles() ?? [];
}