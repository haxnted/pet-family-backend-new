...
public static class AuthenticationExtensions
{
	...
	private static string GetAuthority(IConfiguration configuration)
	{
		...
		var realm = configuration["Keycloak:Realm"]
					?? throw new InvalidOperationException("Keycloak:Realm is required when using Keycloak:Url");
	...
	private static Task OnTokenValidated(TokenValidatedContext context)
	{
		...
		{
			logger?.LogWarning(
				"realm_access roles not found in JWT token. SecurityToken type: {TokenType}",
		...
		logger?.LogInformation(
			"JWT validated. UserId: {Sub}, Roles: [{Roles}]",
			sub,
			string.Join(
	...
	private static bool TryAddRolesFromJson(
		string json,
	...
	private static bool TryAddRolesFromJsonElement(
		JsonElement element,
		ClaimsIdentity identity,
		ILogger? logger)
	{
		try
		{
			...
			foreach (var roleValue in rolesElement.EnumerateArray()
						.Select(role => role.GetString())
						.Where(roleValue => !string.IsNullOrEmpty(roleValue)))
			{
				if (roleValue != null)
				{
					identity.AddClaim(new(ClaimTypes.Role, roleValue));
				}
