...
public class GlobalExceptionHandler(ILogger<GlobalExceptionHandler> logger) : IExceptionHandler
{
	...
	private static ProblemDetails MapExceptionToProblemDetails(Exception exception, HttpContext httpContext)
	{
		...
		{
			var _ when IsDomainException(exception) => CreateValidationProblemDetails(exception, httpContext),
			var _ when IsEntityNotFoundException(exception) => CreateNotFoundProblemDetails(exception, httpContext),
			var _ when IsForbiddenException(exception) => CreateForbiddenProblemDetails(exception, httpContext),
			var _ when IsUseCaseException(exception) => CreateUseCaseProblemDetails(exception, httpContext),
			var _ => CreateInternalServerErrorProblemDetails(exception, httpContext)
		};
	}
	...
	private static ProblemDetails CreateNotFoundProblemDetails(Exception exception, HttpContext httpContext)
	{
		...
		var entityId = entityIdProperty?.GetValue(exception)
			?.ToString();

		var entityName = entityNameProperty?.GetValue(exception)
			?.ToString();
