...
public sealed class ShelterName : ValueObject, IComparable<ShelterName>
{
	...
	public static ShelterName Of(string name)
	{
		...
		{
			throw new DomainException($"{name} содержит недопустимые символы. Разрешены буквы, цифры, пробелы и дефис.");
		}

		if (name.Length is < MinLength or > MaxLength)
		{
			throw new DomainException($"Название приюта не должно превышать {MaxLength} и не быть меньше {MinLength} символов.");
		}
