...
public class PetAdoptionSagaTests : IAsyncLifetime
{
	...
	public async Task StartPetAdoption_ShouldTransitionToReserving_AndPublishReservePet()
	{
		...
		var existsId = await _sagaHarness.Exists(
			_correlationId, x => x.Reserving,
		...
		var result = await _harness.Published.Any<ReservePet>(x => x.Context.Message.CorrelationId == _correlationId
	...
	public async Task PetReserved_ShouldTransitionToCreatingChat_AndPublishCreateAdoptionChat()
	{
		...
		var existsId = await _sagaHarness.Exists(
			_correlationId, x => x.CreatingChat,
			timeout: TimeSpan.FromSeconds(5));

		existsId.HasValue.Should().BeTrue();

		// Assert — CreateAdoptionChat command published
		var result = await _harness.Published.Any<CreateAdoptionChat>(x => x.Context.Message.CorrelationId == _correlationId
																			&& x.Context.Message.PetId == _petId
	...
	public async Task PetReservationFailed_ShouldFinalize_WithFailureReason()
	{
		...
		var notExists = await _sagaHarness.NotExists(
			_correlationId,
			timeout: TimeSpan.FromSeconds(5));
	...
	public async Task AdoptionChatCreated_ShouldTransitionToWaitingForAdoption_AndSendNotifications()
	{
		...
		var existsId = await _sagaHarness.Exists(
			_correlationId, x => x.WaitingForAdoption,
			timeout: TimeSpan.FromSeconds(5));

		existsId.HasValue.Should().BeTrue();

		// Assert — ChatId stored in saga state
		var instance = _sagaHarness.Sagas.ContainsInState(_correlationId, _sagaHarness.StateMachine, x => x.WaitingForAdoption);
		instance.Should().NotBeNull();
	...
	public async Task AdoptionChatCreationFailed_ShouldCompensate_AndPublishUnreservePet()
	{
		...
		// Act
		await _harness.Bus.Publish(
			new AdoptionChatCreationFailed
			{
				CorrelationId = _correlationId,
				PetId = _petId,
				Reason = reason
		...
		var existsId = await _sagaHarness.Exists(
			_correlationId, x => x.Compensating,
			timeout: TimeSpan.FromSeconds(5));

		existsId.HasValue.Should().BeTrue();

		// Assert — failure reason stored
		var instance = _sagaHarness.Sagas.ContainsInState(_correlationId, _sagaHarness.StateMachine, x => x.Compensating);
		instance.Should().NotBeNull();
		instance!.FailureReason.Should().Be(reason);

		// Assert — UnreservePet compensation command published
		var result = await _harness.Published.Any<UnreservePet>(x => x.Context.Message.CorrelationId == _correlationId
																	&& x.Context.Message.PetId == _petId);
	...
	public async Task ConfirmAdoption_ShouldTransitionToAdopting_AndPublishAdoptPet()
	{
		...
		var existsId = await _sagaHarness.Exists(
			_correlationId, x => x.Adopting,
			timeout: TimeSpan.FromSeconds(5));

		existsId.HasValue.Should().BeTrue();

		// Assert — AdoptPet command published
		var result = await _harness.Published.Any<AdoptPet>(x => x.Context.Message.CorrelationId == _correlationId
																&& x.Context.Message.PetId == _petId
																&& x.Context.Message.VolunteerId == _volunteerId
																&& x.Context.Message.AdopterId == _adopterId);
	...
	public async Task PetAdopted_ShouldFinalize_AndSendNotification()
	{
		...
		var result = await _harness.Published.Any<NotificationEvent>(x => x.Context.Message.UserId == _adopterId
	...
	public async Task FullHappyPath_ShouldCompleteSuccessfully()
	{
		...
		(await _sagaHarness.Exists(
				_correlationId, x => x.Reserving,
				timeout: TimeSpan.FromSeconds(5))).HasValue.Should()
			.BeTrue();
	...
	public async Task FullCompensationPath_RejectAdoption_ShouldRollbackCorrectly()
	{
		...
		(await _harness.Published.Any<UnreservePet>(x => x.Context.Message.CorrelationId == _correlationId))
			.Should()
			.BeTrue();
	}
