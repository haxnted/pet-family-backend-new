	...
	: VolunteerManagementIntegrationTestBase(factory)
{
	...
	public async Task ChangeShelterStatus_ShouldUpdate()
	{
		...
		{
			var service = sp.GetRequiredService<IShelterService>();
			await service.ChangeStatusAsync(shelter.Id.Value, (int) ShelterStatus.TemporaryClosed, CancellationToken.None);
		});
	...
	public async Task AssignVolunteer_ShouldAddAssignment()
	{
		...
		{
			var service = sp.GetRequiredService<IShelterService>();
			await service.AssignVolunteerAsync(shelter.Id.Value, volunteerId, (int) VolunteerRole.Caretaker, CancellationToken.None);
		});
