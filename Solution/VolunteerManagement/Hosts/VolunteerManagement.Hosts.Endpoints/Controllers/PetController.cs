	...
	ICurrentUser currentUser) : ControllerBase
{
		...
		CancellationToken ct)
	{
		...
		return Accepted(
			...
			{
				SagaId = sagaId,
				Message = "Процесс усыновления инициирован"
			});
	}
	...
	public async Task<IActionResult> GetAdoptionStatus(Guid sagaId, CancellationToken ct)
	{
		...
		if (result == null)
			return NotFound(
				new
				{
					Message = "Сага усыновления не найдена"
				});
	...
	public async Task<IActionResult> ConfirmAdoption(Guid sagaId, CancellationToken ct)
	{
		...
		return Ok(
			new
			{
				Message = "Подтверждение усыновления отправлено"
			});
	}
		...
		CancellationToken ct)
	{
		...
		return Ok(
			new
			{
				Message = "Отклонение усыновления отправлено"
			});
	}
}